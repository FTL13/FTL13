			...
			if(can_control(usr, R))
				...
				if(choice == "Confirm" && can_control(usr, R) && !..())
					if(R.syndicate && R.emagged)
						...
						if(R.connected_ai)
							R.to_chat(connected_ai, "<br><br><span class='alert'>ALERT - Cyborg detonation detected: [R.name]</span><br>")
		...
		if(src.allowed(usr))
			...
			if(can_control(usr, R))
				...
				if(choice == "Confirm" && can_control(usr, R) && !..())
					...
					if(R.connected_ai)
						R.to_chat(connected_ai, "[!R.lockcharge ? "<span class='notice'>NOTICE - Cyborg lockdown lifted" : "<span class='alert'>ALERT - Cyborg lockdown detected"]: <a href='?src=\ref[R.connected_ai];track=[html_encode(R.name)]'>[R.name]</a></span><br>")
