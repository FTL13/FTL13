	...
	switch(state)
		...
		if(3)
			if(istype(P, /obj/item/weapon/wirecutters))
				if (brain)
					to_chat(user, "<span class='warning'>Get that brain out of there first!</span>")
				else
...
=======
					...
					to_chat(user, "<span class='warning'>You need two sheets of reinforced glass to insert them into AI core!</span>")
				return
			...
			if(istype(P, /obj/item/device/mmi))
				...
				if(!M.brainmob.client)
					to_chat(user, "<span class='warning'>Sticking an inactive brain into the frame would sort of defeat the purpose.</span>")
					return
				...
				if(!M.brainmob.mind)
					to_chat(user, "<span class='warning'>This MMI is mindless!</span>")
>>>>>>> master
					return
...
=======
				...
				M.loc = src
				brain = M
				to_chat(user, "<span class='notice'>Added a brain.</span>")
				icon_state = "3b"
				return
...
/obj/structure/AIcore/deactivated/attackby(obj/item/A, mob/user, params)
	if(istype(A, /obj/item/device/aicard))//Is it?
		...
		switch(anchored)
			if(0)
				if(do_after(user, 20, target = src))
					to_chat(user, "<span class='notice'>You fasten the core into place.</span>")
					anchored = 1
			if(1)
				if(do_after(user, 20, target = src))
					to_chat(user, "<span class='notice'>You unfasten the core.</span>")
					anchored = 0
	else
		return ..()
>>>>>>> master
