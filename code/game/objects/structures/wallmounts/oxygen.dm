...
/obj/structure/oxygen_cabinet/attackby(obj/item/I, mob/user, params)			// Reaction to getting attacked
	if(istype(I, /obj/item/weapon/wrench) && !has_oxygen && !has_mask)			// If wrench and cabinet empty, disassemble
...
/obj/structure/oxygen_cabinet/update_icon()			// Updates sprite based on contents.
	if(!opened)
		icon_state = "oxygen_closed"
	else
		icon_state = "oxygen_[has_oxygen]_[has_mask]"
