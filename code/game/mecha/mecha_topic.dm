...
/obj/mecha/Topic(href, href_list)
	...
	if(in_range(src, usr))
		...
		if(href_list["req_access"] && add_req_access)
			output_access_dialog(mecha_filter.getObj("id_card"),mecha_filter.getMob("user"))

		if(href_list["maint_access"] && maint_access)
			var/mob/user = mecha_filter.getMob("user")
			if(user)
				if(state==0)
					state = 1
				...
				output_maintenance_dialog(mecha_filter.getObj("id_card"),user)
		...
		if(href_list["add_req_access"] && add_req_access && mecha_filter.getObj("id_card"))
			operation_req_access += mecha_filter.getNum("add_req_access")
			output_access_dialog(mecha_filter.getObj("id_card"),mecha_filter.getMob("user"))

		if(href_list["del_req_access"] && add_req_access && mecha_filter.getObj("id_card"))
			operation_req_access -= mecha_filter.getNum("del_req_access")
			output_access_dialog(mecha_filter.getObj("id_card"),mecha_filter.getMob("user"))
	...
	if(href_list["select_equip"])
		var/obj/item/mecha_parts/mecha_equipment/equip = mecha_filter.getObj("select_equip")
	...
	if(href_list["rfreq"])
		var/new_frequency = (radio.frequency + mecha_filter.getNum("rfreq"))
