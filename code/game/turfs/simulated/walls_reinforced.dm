	...
	if(istype(W, /obj/item/weapon/pickaxe/drill/jackhammer))
		...
		if(do_after(user, 50, target = src))
			if( !istype(src, /turf/closed/wall/r_wall) || !user || !W || !T )
				return 1
			if( user.loc == T && attack_hand(user) == W )
				D.playDigSound()
		...
		if (MS.get_amount() < 1)
			to_chat(user, "<span class='warning'>You need one sheet of metal to repair the wall!</span>")
			return 1
		...
		if(1)
			if (istype(W, /obj/item/weapon/screwdriver))
				...
				if(do_after(user, 40, target = src))
					...
					if( d_state == 1 && user.loc == T && attack_hand(user) == W )
						src.d_state = 2
						update_icon()
		...
		if(2)
			if( istype(W, /obj/item/weapon/weldingtool) )
				...
				if( WT.remove_fuel(0,user) )
					...
					if(do_after(user, 60, target = src))
						...
						if( d_state == 2 && user.loc == T && attack_hand(user) == WT )
							src.d_state = 3
							update_icon()
			...
			if( istype(W, /obj/item/weapon/gun/energy/plasmacutter) )
				...
				if(do_after(user, 60, target = src))
					...
					if( d_state == 2 && user.loc == T && attack_hand(user) == W )
						src.d_state = 3
						update_icon()
						to_chat(user, "<span class='notice'>You press firmly on the cover, dislodging it.</span>")
				return 1
		...
		if(3)
			if (istype(W, /obj/item/weapon/crowbar))
				...
				if(do_after(user, 100, target = src))
					...
					if( d_state == 3 && user.loc == T && attack_hand(user) == W )
						src.d_state = 4
						update_icon()
		...
		if(4)
			if (istype(W, /obj/item/weapon/wrench))
				...
				if(do_after(user, 40, target = src))
					...
					if( d_state == 4 && user.loc == T && attack_hand(user) == W )
						src.d_state = 5
						update_icon()
		...
		if(5)
			if( istype(W, /obj/item/weapon/weldingtool) )
				...
				if( WT.remove_fuel(0,user) )
					...
					if(do_after(user, 100, target = src))
						...
						if( d_state == 5 && user.loc == T && attack_hand(user) == WT )
							src.d_state = 6
							update_icon()
			...
			if( istype(W, /obj/item/weapon/gun/energy/plasmacutter) )
				...
				if(do_after(user, 70, target = src))
					...
					if( d_state == 5 && user.loc == T && attack_hand(user) == W )
						src.d_state = 6
						update_icon()
						to_chat(user, "<span class='notice'>You slice through the support rods.</span>")
				return 1
