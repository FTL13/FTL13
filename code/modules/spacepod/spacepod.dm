...
/obj/spacepod/proc/deal_damage(var/damage)
	...
	if(!health)
		spawn(0)
			...
			if(passengers)
				for(var/mob/M in passengers)
					M << "<span class='userdanger'>Critical damage to the vessel detected, core explosion imminent!</span>"
			for(var/i = 10, i >= 0; --i)
				if(armor_stage == 0)
					var/datum/effect_system/smoke_spread/smoke = new
					smoke.set_up(0, src)
					smoke.start()
					for(var/mob/M in passengers)
						M << "<span class='boldnotice'>Gas vented. Explosion averted.</span>"
					return
				if(passengers)
					for(var/mob/M in passengers)
						M << "<span class='warning'>[i]</span>"
				if(i == 0)
					explosion(loc, 2, 4, 8)
					qdel(src)
				sleep(10)
...
/obj/spacepod/emp_act(severity)
	...
	switch(severity)
		if(1)
			if(pilot)
				pilot << "<span class='warning'>The pod console flashes 'Heavy EMP WAVE DETECTED'.</span>"
			if(passengers)
				for(var/mob/M in passengers)
					M << "<span class='warning'>The pod console flashes 'Heavy EMP WAVE DETECTED'.</span>"//warn the passengers
		...
		if(2)
			if(pilot)
				pilot << "<span class='warning'>The pod console flashes 'EMP WAVE DETECTED'.</span>"
			if(passengers)
				for(var/mob/M in passengers)
					M << "<span class='warning'>The pod console flashes 'EMP WAVE DETECTED'.</span>"//warn the passengers
...
/obj/spacepod/attackby(obj/item/W as obj, mob/user as mob, params)
	if(istype(W, /obj/item/weapon/weldingtool) && armor_stage == 0)
		...
		if(!WT.isOn())
			...
			return
		user << "<span class='notice'>You start unwelding the bulkhead panelling...</span>"
		playsound(loc, 'sound/items/Welder2.ogg', 50, 1)
		if(do_after(user, 20, target = src))
			if(!src || !WT.remove_fuel(3, user)) return
			user << "<span class='notice'>You unweld the bulkhead panelling</span>"
	...
	if(armor_stage == 2 && istype(W, /obj/item/weapon/weldingtool))
		...
		if(!WT.isOn())
			user << "<span class='warning'>The welder must be on for this task.</span>"
			return
		user << "<span class='notice'>You start welding the armor...</span>"
		playsound(loc, 'sound/items/Welder2.ogg', 50, 1)
		if(do_after(user, 20, target = src))
			if(!src || !WT.remove_fuel(3, user)) return
			user << "<span class='notice'>You weld the armor</span>"
			armor_stage = 3
			hatch_open = 1
		return
	if(armor_stage < 3)
	...
	if(istype(W, /obj/item/weapon/weldingtool) && hatch_open && !battery && equipment_system.installed_modules.len == 0 && !pilot && passengers.len == 0) // Completely empty pods can have their armor removed
		...
		if(!WT.isOn())
			user << "<span class='warning'>The welder must be on for this task.</span>"
			return
		user << "<span class='notice'>You start unwelding the armor...</span>"
	...
	if(istype(W, /obj/item/weapon/crowbar))
		if(!equipment_system.lock_system || unlocked || hatch_open)
			...
			user << "<span class='warning'>The hatch is locked shut!</span>"
		return
	...
	if(istype(W, /obj/item/device/spacepod_key) && istype(equipment_system.lock_system, /obj/item/device/spacepod_equipment/lock/keyed))
		...
		if (key.id == equipment_system.lock_system.id)
			...
			user << "<span class='warning'>This is the wrong key!</span>"
			return
	...
	if(istype(W, /obj/item/device/lock_buster))
		...
		if(L.on & equipment_system.lock_system)
			...
			return
		user << "<span class='notice'>Turn the [L] on first.</span>"
		return
...
obj/spacepod/proc/add_equipment(mob/user, var/obj/item/device/spacepod_equipment/SPE, var/slot)
	if(equipment_system.vars[slot])
		user << "<span class='notice'>The pod already has a [slot], remove it first.</span>"
		return
	else
		user << "<span class='notice'>You insert \the [SPE] into the pod.</span>"
		user.drop_item(SPE)
		SPE.forceMove(src)
	...
	if(!hatch_open)
		if(cargo_hold.storage_slots > 0)
			if(unlocked)
				...
				user << "<span class='notice'>The storage compartment is locked</span>"
		return ..()
	if(!equipment_system || !istype(equipment_system))
		user << "<span class='warning'>The pod has no equpment datum, or is the wrong type, yell at IK3I.</span>"
		return
...
/obj/spacepod/proc/remove_equipment(mob/user, var/obj/item/device/spacepod_equipment/SPE, var/slot)
	...
	if(passengers.len > max_passengers - SPE.occupant_mod)
		user << "<span class='warning'>Someone is sitting in \the [SPE]!</span>"
		return
	...
	if(cargo_hold.contents.len > cargo_hold.storage_slots - SPE.storage_mod["slots"] || sum_w_class > cargo_hold.max_combined_w_class - SPE.storage_mod["w_class"])
		user << "<span class='warning'>Empty \the [SPE] first!</span>"
		return
...
/obj/spacepod/syndicate/New()
	...
	icon_state = installed_armor.pod_icon_state
	armor_stage = 3
	var/obj/item/device/spacepod_equipment/weaponry/laser/T = new /obj/item/device/spacepod_equipment/weaponry/taser
	T.loc = equipment_system
	equipment_system.weapon_system = T
	equipment_system.weapon_system.my_atom = src
	equipment_system.installed_modules += T
...
/obj/spacepod/verb/toggle_internal_tank()
	...
	usr << "<span class='notice'>Now taking air from [use_internal_tank?"internal airtank":"environment"].</span>"
	...
	if(istype(A,/mob))
		...
		if(M != user && unlocked && (M.stat == DEAD || M.incapacitated()))
			if(passengers.len >= max_passengers && !pilot)
				usr << "<span class='danger'><b>That person can't fly the pod!</b></span>"
				return 0
			if(passengers.len < max_passengers)
...
/obj/spacepod/proc/load_cargo(mob/user, var/obj/O)
	...
	if(!C.storage)
		...
		user << "<span class='warning'>\The [src] already has \an [C.storage]</span>"
...
/obj/spacepod/proc/enter_pod(mob/user)
	...
	if(equipment_system.lock_system && !unlocked)
		user << "<span class='warning'>\The [src]'s doors are locked!</span>"
		return 0
	...
	if(passengers.len <= max_passengers)
		...
		if(do_after(user, 40, target = src))
			...
			if(passengers.len < max_passengers)
				...
				user << "<span class='notice'>You were too slow. Try better next time, loser.</span>"
		else
...
/obj/spacepod/verb/lock_pod()
	...
	unlocked = !unlocked
	usr << "<span class='warning'>You [unlocked ? "unlock" : "lock"] the doors.</span>"
...
/obj/spacepod/proc/lightsToggle()
	...
	if(lights)
		...
		SetLuminosity(0)
	usr << "Lights toggled [lights ? "on" : "off"]."
	for(var/mob/M in passengers)
		M << "Lights toggled [lights ? "on" : "off"]."
...
/obj/spacepod/verb/checkSeat()
	...
	set src = usr.loc
	var/mob/user = usr
	user << "<span class='notice'>You start rooting around under the seat for lost items</span>"
	if(do_after(user, 40, target = src))
		...
		if(true_contents.len > 0)
			...
			if(user.put_in_any_hand_if_possible(I))
				src.contents -= I
				user << "<span class='notice'>You find a [I] [pick("under the seat", "under the console", "in the mainenance access")]!</span>"
			else
				user << "<span class='notice'>You think you saw something shiny, but you can't reach it!</span>"
		else
			user << "<span class='notice'>You fail to find anything of value.</span>"
	else
		user << "<span class='notice'>You decide against searching the [src]</span>"
