...
#define SPEAK(message) radio.talk_into(src, message)
...
/obj/machinery/power/supermatter_shard/attack_hand(mob/living/user)
	if(!istype(user))
		return
	user.visible_message("<span class='danger'>\The [user] reaches out and touches \the [src], inducing a resonance... their body starts to glow and bursts into flames before flashing into ash.</span>",\
...
/obj/machinery/power/supermatter_shard/Bumped(atom/AM)
	if(isliving(AM))
		AM.visible_message("<span class='danger'>\The [AM] slams into \the [src] inducing a resonance... their body starts to glow and catch flame before flashing into ash.</span>",\
