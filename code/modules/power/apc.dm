...
/obj/machinery/power/apc/proc/malfhacked(mob/living/silicon/ai/malf)
	...
	if(src && !src.aidisabled)
		...
		to_chat(malf, "Hack complete. The APC is now under your exclusive control.")
		update_icon()
	else
		to_chat(malf, "Hack aborted. The designated APC has stopped responding and no longer exists on the power network.")
...
/obj/machinery/power/apc/proc/malfvacate(forced)
	...
	if(src.occupier.parent && src.occupier.parent.stat != 2)
		...
		src.to_chat(occupier, "<span class='danger'>Primary core damaged, unable to return core processes.</span>")
		if(forced)
...
/obj/machinery/power/apc/process()
	...
	if(lastused_total > 0)
		longtermpowercache = lastused_total
	power_requested = longtermpowercache
	...
	if(operating)
		if(last_power_received != 0)
			percent = (longtermpowercache / last_power_received) * 100
	...
	else if(percent > 1 && percent <= 15)	// <15%, turn off lighting & equipment
		equipment = autoset(equipment, 2)
		lighting = autoset(lighting, 2)
		environ = autoset(environ, 1)
		area.poweralert(0, src)
	else if(percent > 15 && percent <= 30)			// <30%, turn off equipment
		equipment = autoset(equipment, 2)
