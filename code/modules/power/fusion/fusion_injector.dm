/obj/machinery/injector //The injector is must be constructed as part of the containment pipe loop
...
/obj/machinery/injector/Destroy()
	qdel(atmos_terminal)
	atmos_terminal = null
	qdel(fusion_pipe)
	fusion_pipe = null
...
/obj/machinery/injector/attackby(obj/item/I, mob/user, params)
...
/obj/machinery/injector/process_atmos()
	...
	if(atmos_terminal.NODE1 && atmos_terminal.AIR1)
		if("hydrogen" in cached_gases && cached_gases["hydrogen"][MOLES] && cached_gases["hydrogen"][MOLES] < yield && energy < yield && remaining < yield)
			cached_gases["hydrogen"][MOLES] -= yield
			energy -= yield
			remaining -= yield
