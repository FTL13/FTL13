...
// We skip `add_to_transit` here because we want to skip the checks in order to save time
	// Bottom border
	for(var/turf/open/space/S in block(locate(1,1,zpos),locate(world.maxx,TRANSITIONEDGE+1,zpos)))
		transit_south |= S

	// Top border
	for(var/turf/open/space/S in block(locate(1,world.maxy,zpos),locate(world.maxx,world.maxy - TRANSITIONEDGE - 1,zpos)))
		transit_north |= S

	// Left border
	for(var/turf/open/space/S in block(locate(1,TRANSITIONEDGE+1,zpos),locate(TRANSITIONEDGE+1,world.maxy - TRANSITIONEDGE - 2,zpos)))
		transit_west |= S
