...
/datum/space_level/proc/build_space_destination_arrays()
	...
	// Bottom border
	for(var/turf/space/S in block(locate(1,1,zpos),locate(world.maxz,TRANSITIONEDGE,zpos)))
		transit_south |= S

	// Top border
	for(var/turf/space/S in block(locate(1,world.maxy,zpos),locate(world.maxz,world.maxy - TRANSITIONEDGE - 1,zpos)))
		transit_north |= S

	// Left border
	for(var/turf/space/S in block(locate(1,TRANSITIONEDGE+1,zpos),locate(TRANSITIONEDGE,world.maxy - TRANSITIONEDGE - 2,zpos)))
		transit_west |= S
