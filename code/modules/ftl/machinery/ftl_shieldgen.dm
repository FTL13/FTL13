/obj/machinery/ftl_shieldgen
	...
	var/load = 50000 //Power needed to keep the shield running
...
/obj/machinery/ftl_shieldgen/process()
	...
	if(power_charge < power_charge_max)		// if there's power available, try to charge
		...
		if(power_terminal.load() * GLOB.CELLRATE <= 0)
			power_charge += power_charge_max-power_charge
		else
			power_charge += min(power_charge_max-power_charge, min(power_terminal.surplus() * GLOB.CELLRATE, 1000))
		charging_power = 1
	else
		charging_power = 0
